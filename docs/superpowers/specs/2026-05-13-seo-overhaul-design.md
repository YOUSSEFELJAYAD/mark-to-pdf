...
app/
  ...
  icon.tsx                            (new) dynamic favicon (32×32)
  apple-icon.tsx                      (new) dynamic apple-touch-icon (180×180)
  icon-192.png and icon-512.png       (new — generated by app/icon.tsx size variants)
...
- `app/icon.tsx` — 32×32 favicon, ImageResponse with monogram "M" on the brand color
- `app/apple-icon.tsx` — 180×180 same
- For manifest's 192/512 icons: create `app/icon.tsx` with `export const sizes = [{ width: 32, height: 32 }, { width: 192, height: 192 }, { width: 512, height: 512 }]` … actually Next.js file-conventions don't take a sizes array — each size needs its own filename: `icon.tsx` (default 32), `icon-192.png` (static or via metadata route), `icon-512.png`. **Decision:** generate `icon-192.png` and `icon-512.png` by hand via a Node script that uses `ImageResponse` or just inline `<svg>` rendered to PNG. Simpler: drop the 192/512 icons from `manifest.json` and rely on the `icon.tsx` 32×32 + `apple-icon.tsx` 180×180. Manifest still gets `icons: []` empty array (PWA install will use the favicon). Acceptable for a free tool — not a primary install target.

Final asset set:

| File | Source | Size | Purpose |
|---|---|---|---|
| `app/opengraph-image.tsx` | dynamic | 1200×630 | OG cards |
| `app/twitter-image.tsx` | dynamic | 1200×630 | Twitter cards |
...
`manifest.json` updated to remove the 192/512 references and to point `icons: [{ src: "/icon", sizes: "32x32" }, { src: "/apple-icon", sizes: "180x180" }]`. (Next.js serves these from the file conventions at `/icon` and `/apple-icon`.)
...
> **Query-param hand-off:** `app/page.tsx` checks `searchParams.example` (Next.js 16 server search params), maps to a built-in example markdown, and passes via prop to the client `MarkdownConverter`. Converter only adopts if `markdown === SAMPLE_MARKDOWN` (i.e., user hasn't customized yet).

## Phase 4 — Performance

### 4.1 Lazy-load jspdf and docx

In `components/markdown-converter.tsx`, change `handleExport` to dynamically import the export module:
...
This removes ~250KB (jspdf + docx) from the initial JS bundle. They're loaded the first time the user clicks Export — usually after they've already engaged with the editor for at least a few seconds, so the perceived latency is negligible.
...
### 4.4 CodeMirror dynamic import (verify)

The current `MarkdownEditor` is a `'use client'` component but is statically imported. Wrap the import in `next/dynamic` with `{ ssr: false }` inside `MarkdownConverter`:

```ts
const MarkdownEditor = dynamic(() => import("@/components/editor/markdown-editor").then(m => m.MarkdownEditor), { ssr: false })
```

This keeps CodeMirror out of the SSR HTML payload. The editor card shows a small skeleton on first paint, then CodeMirror loads. Bundle moves from initial chunk to a route-level chunk.

> **Tradeoff:** dynamic import + `ssr: false` means the editor doesn't appear in the SSR HTML. SEO doesn't care (the editor isn't keyword content), but the perceived first paint shows the empty card briefly. Acceptable.
...
1. **`SoftwareApplication` requires `offers`** for free-software rich results; included. `aggregateRating` not included — no actual ratings. Adding fake ratings = manual penalty.
2. **`HowTo` rich results were deprecated by Google in 2023 for some non-English locales** but still display in EN-US for many queries. Keeping it.
3. **`FAQPage` rich results were downgraded in 2023** — only shown for "well-known, authoritative sites." Still worth doing; sometimes triggers anyway, and the schema is valid for users who view source.
4. **Dynamic OG image latency** — `next/og` `ImageResponse` runs on-edge in Vercel's runtime; ~50ms cold. First social share crawler may see slightly delayed image, but most cache it.
5. **`/examples` query-param hand-off** is mildly clever but limited: only works if home editor still has sample content. Acceptable — power users who'd want to import an example already have a non-sample doc.
6. **Lazy-loading export libs adds a small loading delay** on first export click (one bundle fetch, ~50–200ms on cable). The Export button already shows "Generating PDF…", so this is masked.
